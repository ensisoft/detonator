...
-- Entity 'bee' script.
--
-- This script will be called for every instance of 'bee' in the scene
-- during gameplay.
-- You're free to delete functions you don't need.
--
-- Called once when the game play begins for the entity in the scene.
function BeginPlay(bee, scene, map)
    if bee.action == 'fly-right' then
        local bee_node = bee:GetNode(0)
        local bee_draw = bee_node:GetDrawable()
        bee_draw:SetFlag('FlipHorizontally', true)
    end
end

-- Called once when the game play ends for the entity in the scene.
function EndPlay(bee, scene, map)
end
...
-- path finding etc.
function Tick(bee, game_time, dt)
    local dino = Scene:FindEntity('Player')
    ...
    local bee_node = bee:GetNode(0)
    local bee_translation = bee_node:GetTranslation()

    local dino_node = dino:GetNode(0)
    local dino_translation = dino_node:GetTranslation()
    local distance = math.abs(bee_translation.x - dino_translation.x)
    local altitude = math.abs(bee_translation.y - dino_translation.y)

    if distance < 1000.0 and altitude < 200.0 then
        local bee_rigid_body = bee_node:GetRigidBody()
        if bee.action == 'fly-left' then
            bee_rigid_body:AdjustLinearVelocity(-8.0, 0.0)
        elseif bee.action == 'fly-right' then
            bee_rigid_body:AdjustLinearVelocity(8.0, 0.0)
        end
    end

    if bee.action == 'fly-left' then
        if bee_translation.x < dino_translation.x and distance > 500.0 then
            Game:DebugPrint('Bee died')
            bee:Die()
        end
    elseif bee.action == 'fly-right' then
        if bee_translation.x > dino_translation.x and distance > 500.0 then
    ...
    if distance < -500.0 and bee_translation.x < dino_translation.x then
        Game:DebugPrint('Bee died')
        bee:Die()
    end
...
-- Called on every iteration of the game loop. game_time is the current
-- game time so far in seconds not including the next time step dt.
function Update(bee, game_time, dt)
end
...
function PostUpdate(bee, game_time)
end
...
function OnBeginContact(bee, node, other_entity, other_node)
end

-- Similar to OnBeginContact except this happens when the contact ends.
function OnEndContact(bee, node, other_entity, other_node)
end
...
function OnKeyDown(bee, symbol, modifier_bits)
end

-- Called on key up events. See OnKeyDown for more details.
function OnKeyUp(bee, symbol, modifier_bits)
end
...
function OnTimer(bee, timer, jitter)
end
