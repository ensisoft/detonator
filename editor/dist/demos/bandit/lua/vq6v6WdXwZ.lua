...
function OnBeginContact(player, node, other, other_node)
    ...
    Game:DebugPrint(util.FormatString('Begin %1 contact with %2', body_part,
                                      object))
    ...
    if body_part == 'Left Collider' then
        if object == 'Plateu' or object == 'Small Island' or object ==
            'StoneBlock' then
...
function OnEndContact(player, node, other, other_node)
    ...
    Game:DebugPrint(util.FormatString('End %1 conctact with %2', body_part,
                                      object))
