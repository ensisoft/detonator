--
-- Entity 'brick_particles' script.
-- This script will be called for every instance of 'brick_particles' in the scene during gameplay.
-- You're free to delete functions you don't need.
--
-- Called once when the game play begins for the entity in the scene.
function BeginPlay(brick_particles, scene, map)

    -- we set the variable when spawning
    local particle_color = brick_particles.color
    local particle_node = brick_particles:GetNode(0)
    local particle_draw = particle_node:GetDrawable()
    particle_draw:SetUniform('kBaseColor', particle_color)
end

-- Called once when the game play ends for the entity in the scene.
function EndPlay(brick_particles, scene, map)
end
...
-- path finding etc.
function Tick(brick_particles, game_time, dt)
end
...
-- Called on every iteration of the game loop. game_time is the current
-- game time so far in seconds not including the next time step dt.
function Update(brick_particles, game_time, dt)
end
...
function PostUpdate(brick_particles, game_time)
end
...
function OnBeginContact(brick_particles, node, other_entity, other_node)
end

-- Similar to OnBeginContact except this happens when the contact ends.
function OnEndContact(brick_particles, node, other_entity, other_node)
end
...
function OnKeyDown(brick_particles, symbol, modifier_bits)
end

-- Called on key up events. See OnKeyDown for more details.
function OnKeyUp(brick_particles, symbol, modifier_bits)
end
...
function OnTimer(brick_particles, timer, jitter)
end
...
function OnEvent(brick_particles, event)
end
