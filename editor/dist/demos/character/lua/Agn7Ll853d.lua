--
-- Entity 'arrow' script.
-- This script will be called for every instance of 'arrow' in the scene during gameplay.
...
function BeginPlay(arrow, scene, map)
    ...
    local arrow_node = arrow:GetNode(1)
    local shadow_node = arrow:GetNode(2)
    local angle = math.acos(angle)
    if dir.y < 0.0 then
        angle = -angle
    end
    arrow_node:SetRotation(angle)
    shadow_node:SetRotation(angle)
end
...
-- path finding etc.
function Tick(arrow, game_time, dt)
end
...
function Update(arrow, game_time, dt)
    local step = arrow.direction * dt
    local dist = glm.length(step)
    arrow.distance = arrow.distance + dist

    local node = arrow:GetNode(0)
    node:Translate(step)

    if arrow.distance > 500.0 then
        arrow:Die()
    end
end
...
function PostUpdate(arrow, game_time)

    if arrow:IsDying() or arrow:HasBeenKilled() then
        return
    end

    local point_node = arrow:GetNode(3)
    local point = Scene:MapPointFromEntityNode(arrow, point_node,
                                               glm.vec2:new(0.0, 0.0))

    local nodes = Scene:QuerySpatialNodes(point, 5.0, 'Closest')
    while nodes:HasNext() do
        local node = nodes:GetNext()
        local entity = node:GetEntity()
        CallMethod(entity, 'ArrowHit')
        arrow:Die()
        break
    end
end
...
function OnBeginContact(arrow, node, other_entity, other_node)
end

-- Similar to OnBeginContact except this happens when the contact ends.
function OnEndContact(arrow, node, other_entity, other_node)
end
...
function OnKeyDown(arrow, symbol, modifier_bits)
end

-- Called on key up events. See OnKeyDown for more details.
function OnKeyUp(arrow, symbol, modifier_bits)
end
...
function OnTimer(arrow, timer, jitter)
end
