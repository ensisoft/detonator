...
{
    if (mContentWindow) {
        if (mContentWindow->close()) {
            mContentWindow = nullptr;
            close();
        }
    }
}
...
void FramelessWindow::setContent(QMainWindow *window) {
  ui->windowContent->layout()->addWidget(window);

  ui->menu->addWidget(window->menuWidget());
  ui->titleText->setText(window->windowTitle());

  connect(window, &QMainWindow::windowTitleChanged, this, [this](const QString& title) {
...
void FramelessWindow::enableShadow(bool on_off)
{
    m_bEnableShadowEffect = on_off;
}

void FramelessWindow::init()
{
    // shadow under window title text
    if (m_bEnableShadowEffect)
    {
...
void FramelessWindow::on_closeButton_clicked() {
    if (mContentWindow) {
        mContentWindow->close(); // will cause closeEvent
    }
}
