...
void PlayLoopIterate()
{
    ...
    for (int i=0; i<buffers_processed; ++i)
    {
        ...
        CallOpenAL(alBufferData, buffer_handle, AL_FORMAT_STEREO16, &pcm[0], pcm_bytes, 44100);
        CallOpenAL(alSourceQueueBuffers, g_source, 1, &buffer_handle);
    }
}
...
int main()
{
    ...
    CallOpenAL(alGenSources,1, &g_source);
    CallOpenAL(alGenBuffers,5, &g_buffers[0]);
    ...
    AL_CALL(alGenBuffers(1, &buffer_handle));
    AL_CALL(alBufferData(buffer_handle, AL_FORMAT_STEREO16, data, size, 44100));
    AL_CALL(alSourcei(g_source, AL_BUFFER, buffer_handle));
    */
    ...
    std::vector<char> pcm;
    pcm.resize(2048000);
    const auto bytes = GenerateSine(&pcm[0], pcm.size());

    ALuint buffer_handle = 0;
    AL_CALL(alGenBuffers(1, &buffer_handle));
    AL_CALL(alBufferData(buffer_handle, AL_FORMAT_STEREO16, &pcm[0], bytes, 44100));
    AL_CALL(alSourcei(g_source, AL_BUFFER, buffer_handle));
