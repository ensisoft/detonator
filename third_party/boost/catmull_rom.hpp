...
namespace boost{ namespace math{
        ...
        namespace detail
        {
            template<class Point, size_t Dimensions>
        ...
        template <class Point, size_t Dimensions, class RandomAccessContainer = std::vector<Point> >
        class catmull_rom
        {
        ...
        public:
            ...
            catmull_rom(std::initializer_list<Point> l, bool closed = false, value_type alpha = (value_type) 1/ (value_type) 2) : catmull_rom<Point, Dimensions, RandomAccessContainer>(RandomAccessContainer(l), closed, alpha) {}
            ...
            {
                return m_pnts.size() >= 4;
            }

            Point evaluate(const value_type s) const
            {
                return (*this)(s);
            }
        ...
        template<class Point, size_t Dimensions, class RandomAccessContainer >
        catmull_rom<Point, Dimensions, RandomAccessContainer>::catmull_rom(RandomAccessContainer&& points, bool closed, typename Point::value_type alpha) : m_pnts(std::move(points))
        {
            ...
            m_s[0] = -detail::alpha_distance<Point, Dimensions>(m_pnts[0], m_pnts[1], alpha);
            ...
            {
                typename Point::value_type d = detail::alpha_distance<Point, Dimensions>(m_pnts[i], m_pnts[i-1], alpha);
        ...
        template<class Point, size_t Dimensions, class RandomAccessContainer >
        Point catmull_rom<Point, Dimensions, RandomAccessContainer>::operator()(const typename Point::value_type s) const
        {
            assert(is_valid());
        ...
        template<class Point, size_t Dimensions, class RandomAccessContainer >
        Point catmull_rom<Point, Dimensions, RandomAccessContainer>::prime(const typename Point::value_type s) const
        {
            using std_workaround::size;
