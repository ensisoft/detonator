...
void FragmentShaderMain() {
    ...
    if (TestMap(BASIC_LIGHT_MATERIAL_DIFFUSE_MAP)) {
        vec2 diffuse_map_coords = texture_coords * kDiffuseMapRect.zw + kDiffuseMapRect.xy;
    ...
    if (TestMap(BASIC_LIGHT_MATERIAL_SPECULAR_MAP)) {
        vec2 specular_map_coords = texture_coords * kSpecularMapRect.zw + kSpecularMapRect.xy;
    ...
    if (TestMap(BASIC_LIGHT_MATERIAL_NORMAL_MAP)) {
        vec2 normal_map_coords = texture_coords * kNormalMapRect.zw + kNormalMapRect.xy;
